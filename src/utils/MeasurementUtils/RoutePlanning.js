 ...
 * @return {*}
 */
function setCurrentWayPointStyle (watchCurrentWayPointIndex) {
 ...
 * @return {*}
 */
function setCurrentActionOrientation (watchCurrentWayPointIndex, watchCurrentActionIndex, selectWayPoint, actionChangeData, nextWayPoint) {
 ...
 * @return {*}
 */
function setCurrentActionStyle (watchCurrentWayPointIndex, watchCurrentActionIndex) {
  const _entitiesIds = uniq(entitiesIds).filter(_ =>
 ...
 * @return {*}
 */
function lookingDownNew (longitude, latitude, altitude) {
  let _position = {}
 ...
 * @return {*}
 */
function setTrackedEntity (status) {
 ...
 * @return {*}
 */
function makeConeByHeadingPitchRoll (action, point) {
 ...
 * @return {*}
 */
function actionBarsModal (type, value) {
  if (!viewer.entities.getById('movePoint')) {
    return
  }
  const entity = viewer.entities.getById('movePoint')
 ...
 * @return {*}
 */
function clearRouteEntities (idPrefix) {
  let _entitiesIds = uniq(entitiesIds)
  if (idPrefix) {
    _entitiesIds = _entitiesIds.filter(_ =>
      _.indexOf(idPrefix) > -1
  ...
  // if (type && type === 'route') {
  //   if (type === 'route') {
  //     // 只清除航线相关的实体（航线，航点，距离标注）,保留航点球
  //     _entitiesIds = uniq(entitiesIds).filter(_ =>
  //       _.indexOf('routePlanningWayPoint') > -1 ||
  //       _.indexOf('routePlanningHeadingLine') > -1 ||
  //       _.indexOf('routePlanningAltitudeLabel') > -1 ||
  //       _.indexOf('routePlanningPlanningRoute') > -1 ||
  //       _.indexOf('routePlanningDistanceLabel') > -1
  //     )
  //   } else if (type === 'flyToRoute') {
  //     _entitiesIds = uniq(entitiesIds).filter(_ =>
  //       _.indexOf('flyToRoute') > -1
  //     )
  //   }
  // }
  // if (_entitiesIds && _entitiesIds.length > 0) {
 ...
 * @return {*}
 */
function makeWayPointEntity (list, idPrefix, wayPointImage) {
  if (list && list.length > 0) {
    list.map((point, index) => {
      let _label = ''
...
function handleMakeRouteEntity (options) {
  ...
  if (_list && _list.length > 0) {
    ...
    // 绘制航点
    makeWayPointEntity(_list, _idPrefix, _wayPointImage)
    if (_heading) {
      // 绘制航点方向
 ...
 * @return {*}
 * 示例：
  RoutePlanningUtils.makeRouteEntities({
    list: list, // 航点数据
    idPrefix: 'routeList', // 航线所有元素 ID 前缀（用于多处绘制/清除航线）
    fly: true, // 是否定位到航线处
    wayPointImage: require('@/assets/images/controls/routePlanning/pointGreen.png'), // 航点图标
 ...
 */
function makeRouteEntities (options) {
 ...
 * @return {*}
 */
function clearEntityMoveStatus () {
  if (pickedEntity) {
 ...
 * @return {*}
 */
const setMovePointEntityPosition = debounce(function (position) {
  // 高度默认为机库的高度
 ...
 * @return {*}
 */
function setMovePointEntityVisible (status) {
  const entity = viewer.entities.getById('movePoint')
...
export default {
  ...
  handleRouteEntitiesVisible,
  clearRouteEntities,
  flyToRoute,
  init,
  makeConeByHeadingPitchRoll,
  makeCone,
  clearCone,
  computedVoyageByWayPoint,
  removeInputActionFun,
  actionBarsModal,
  setTrackedEntity,
  lookingDownNew,
